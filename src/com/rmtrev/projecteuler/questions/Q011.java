 ...
 *
 */
public class Q011 implements IQuestion
{
	public static int[][] GRID = new int[][]
			{
				new int[] {8, 02, 22, 97, 38, 15, 00, 40, 00, 75, 04, 05, 07, 78, 52, 12, 50, 77, 91, 8},
	...
	public void Run(String[] args) throws Exception
	{
		...
		for(int x = 0; x < 17; x++)
		{
			for(int y = 0; y < 20; y++)
			{
				// only concern ourselves with E, S, SE, and NE directions
				// N, W, SW, and NW are redundant
				long product;
				
				if(y < 17)
				{
					// get SE
					product = getProduct(x, y, 1, 1);
					if(product > maxProduct) maxProduct = product;
					
					// get S
					product = getProduct(x, y, 0, 1);
					if(product > maxProduct) maxProduct = product;
				}
				
				// get E
				product = getProduct(x, y, 1, 0);
				if(product > maxProduct) maxProduct = product;
				
				// if we're far down enough get NE
				if(y >= 3)
				{
					product = getProduct(x, y, 1, -1);
					if(product > maxProduct) maxProduct = product;
		...
		System.out.println(Long.toString(maxProduct));
	}
	
	private long getProduct(int posX, int posY, int dirX, int dirY)
	{
		long ret = 1;
		
		for(int i = 0; i < 4; i++)
		{
			ret *= GRID[posX + i * dirX][posY + i * dirY];
		}
